...
const createCarModel = async (req, res) => {
  try {
    const { brand_id, name, colors } = req.body;
    ...
    const result = await db.query(
      'INSERT INTO cars_products (brand_id, name, image_url, colors, is_active) VALUES ($1, $2, $3, $4, $5) RETURNING *',
      [brand_id, name, imageUrl, JSON.stringify(colorImages), true]
    );
...
const updateCarModel = async (req, res) => {
  try {
    ...
    const result = await db.query(
      'UPDATE cars_products SET brand_id = $1, name = $2, image_url = $3, colors = $4, updated_at = CURRENT_TIMESTAMP WHERE id = $5 RETURNING *',
      [brand_id, name, imageUrl, JSON.stringify(colorImages), id]
    );
